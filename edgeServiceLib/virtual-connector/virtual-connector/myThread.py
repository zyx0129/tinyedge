...
class Thread(threading.Thread):
    ...
    def run(self):
        ...
        while (True):
            ...
            try:
                # _logger.debug("%d, getting ruleTask from threadPool queue...", self.taskQueue.qsize())
                func, kwargs = self.taskQueue.get(block=True, timeout=3)
                func(**kwargs)
                # _logger.debug("thread done")
            except queue.Empty:
                # _logger.info("threaPool Queue is empty and have no ruleTask in the queue")
                pass
            except:
                _logger.exception("threaPool: task exec err#####")
